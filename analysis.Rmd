...
gcf_df <- bigslice_df %>% filter(orig_folder %in% regions$accession_id) %>% select(genus, gcf_id) %>% distinct() %>% count(genus, name = "n_gcfs")
```
...
hiq_genomes_by_genus <- ncbi_hiq_meta %>% left_join(cyano_asm_tax, by = join_by(`Assembly Accession` == assembly_accession)) %>% group_by(genus) %>% summarize(n_genomes = n())
```
