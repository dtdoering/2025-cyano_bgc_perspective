...
# assembly accession and selected tax ranks
# 6.1k entries
cyano_asm_tax <- read_tsv("data/cyano_asm_tax.tsv")
# assembly accession, assembly level, taxid, and select tax ranks
# 10.2k entries
cyano_asm_dataset <- read_tsv("data/cyanos_genomes_taxids.tsv") %>%
  left_join(
    read_tsv("data/taxonomy_summary.tsv"), 
    by = join_by("Organism Taxonomic ID" == "Taxid")
  ) %>%
  select(
    `Assembly Accession`,
    `Assembly Level`,
...
# Table of NCBI cyano assemblies at "chromosome" or "complete" quality levels
ncbi_hiq_meta <- read_tsv("data/ncbi_cyano_HiQualityGenomes_metadata.tsv")

# List of the accessions that have been "seen" by the SMC BGC pipeline, whether BGCs were found or not
# NOTE: Does not include accessions that were found to be MD5-identical (I think)
smc_track_df <- read_tsv("data/smc_tracked_accessions.tsv") %>%
  pivot_longer(
    cols = ends_with("Accession"),
    names_to = "source",
    names_pattern = "([[:alnum:]]+)Accession",
    values_to = "accession"
  ) %>%
  drop_na(accession)

smc_tracked_cyanos_df <- smc_track_df %>% 
  semi_join(ncbi_hiq_meta, by = join_by(accession == `Assembly Accession`)) %>%
  inner_join(cyano_asm_dataset, by = join_by(accession == `Assembly Accession`)) %>% 
  mutate(id_sfx = str_remove(accession, "GC[FA]")) %>% 
  arrange(id_sfx)
...
bigslice_df <- read_tsv("data/2025-03-14-1524-bigslice_results.tsv") %>%
  left_join(
    cyano_asm_dataset, 
    by = join_by("orig_folder" == "Assembly Accession")
  ) %>%
  rename(
...
# Handy vector of accession suffixes (i.e. remove the GCA/GCF from assembly 
# accession) for RefSeq/GenBank assemblies that are identical to each other

# If a RefSeq assembly accession is present in one dataset, but only the
# GenBank assembly was analyzed for another dataset, joining the two datasets 
# can retain them both


# filter huge file for accessions found in cyano-specific `ncbi_hiq_meta`
ident_accession_sfx <- read_tsv("data/assembly_summary_genbank.txt", skip = 1) %>%
  ...
  filter(
    assembly_level %in% c("Complete Genome", "Chromosome"),
    paired_asm_comp == "identical"
  ) %>%
  semi_join(
    ncbi_hiq_meta,
    by = join_by(assembly_accession == `Assembly Accession`)
  ) %>% 
  pull(gbrs_paired_asm) %>% 
  str_remove("GC[AF]")
...
regions_unfiltered <- regions_unfiltered %>%
  mutate(classes = map(region_class, function(class_string) {
    # convert a JSON-string list of BGC classes to an R vector
    # "['NRPS', 'PKS']" -> c('NRPS', 'PKS')
    # if it's just one class, it's not JSON-ified already
    # "NRPS" -> c('NRPS')
    (
  ...
  mutate(categories = classes %>% map(function(cls_vec) {
    # map the BGC classes to their categories and condense
...
# use the vector of identical accession suffixes to deduplicate the datasets
regions_unfiltered <- regions_unfiltered %>% 
  mutate(
    id_sfx = str_remove(accession_id, "GC[AF]"), 
    pair_is_ident = (id_sfx %in% ident_accession_sfx)
  )

bigslice_df <- bigslice_df %>%
  mutate(
    id_sfx = str_remove(orig_folder, "GC[AF]"),
    pair_is_ident = (id_sfx %in% ident_accession_sfx)
  )

smc_tracked_cyanos_df <- smc_tracked_cyanos_df %>%
  mutate(pair_is_ident = (id_sfx %in% ident_accession_sfx))

# For accessions seen by SMC, if they are identical to their pair but the pair 
# accession has not been seen, then add the missing rows to the tracker, 
# swapping GCF <-> GCA as needed

smc_tracked_cyanos_df <- bind_rows(
  smc_tracked_cyanos_df,
  smc_tracked_cyanos_df %>% 
  filter(n() == 1, pair_is_ident == TRUE, .by = id_sfx) %>% 
  mutate(accession = case_when(
    source == "Genbank" ~ str_replace(accession, "GCA", "GCF"), 
    source == "Refseq" ~ str_replace(accession, "GCF", "GCA")
  ))
)
...
There are `r n_distinct(regions_unfiltered$smc_id)` BGC-containing genomes and `r nrow(regions_unfiltered)` BGCs in the initial dataset.
...
regions_unfiltered %>%
  ...
  facet_wrap(
    ...
    labeller = as_labeller(c(
      "FALSE" = "BGCs not on contig edge", 
      "TRUE" = "BGCs on contig edge"
    ))
  ) +
  theme_bw() +
  ggtitle(
    "Fragmented genomes have inflated BGC counts", 
...
In order to normalize BGC counts to a per-genome basis, we must also know how many Cyano genomes are in SMC but *lack* BGCs (as detected by antiSMASH).
...
cyano_nohits <- read_tsv("data/ncbi_cyano_nohit_accs.txt", col_names = c("accession_id")) %>%
  ...
  filter(accession_id %in% ncbi_hiq_meta$`Assembly Accession`)
...
hiq_genomes_by_genus <- ncbi_hiq_meta %>%
  semi_join(smc_tracked_cyanos_df, by = join_by(`Assembly Accession` == accession)) %>%
  left_join(cyano_asm_tax, by = join_by(`Assembly Accession` == assembly_accession)) %>%
  group_by(genus) %>%
  summarize(tot_genomes = n()) %>%
  rename(tax_genus = genus)
...
df_plots <- tax_count %>%
  ...
  left_join(hiq_genomes_by_genus, by = "tax_genus") %>%
...
```{r}
gcf_df <- bigslice_df %>%
  semi_join(regions, by = join_by(orig_folder == accession_id)) %>%
  select(genus, gcf_id) %>%
  distinct() %>%
  count(genus, name = "n_gcfs") %>% arrange(genus)

```

Plot the BGCs per genome (not normalized to 100%) alongside number of BGCs per genus, number of GCFs per genus, and genomes per genus
...
df_plots
p5
ggsave("./figs/png/p_bgc_ct.png", p_bgc_ct, width = 8, height = 11, device = "png")
ggsave("./figs/png/p_genome_ct.png", p_genome_ct, width = 8, height = 11, device = "png")
ggsave("./figs/png/p_gcf_ct.png", p_gcf_ct, width = 8, height = 11, device = "png")
...
p_a <- plot_grid(
  ...
  p_bgc_ct + text_y_off + theme(
    axis.text.x = element_text(size = rel(0.8)),
    axis.ticks.y = element_blank(),
    plot.margin = sm_margin
  ),
  p_gcf_ct + text_y_off + theme(
    axis.text.x = element_text(size = rel(0.8)),
    axis.ticks.y = element_blank(),
    plot.margin = sm_margin
  ),
  p_genome_ct + text_y_off + theme(
    axis.text.x = element_text(size = rel(0.8)),
...
ggsave("./figs/_fig2.svg", fig2, width = 6.73, height = 9.17, units = "in", device = "svg")
ggsave("./figs/_fig2.jpg", fig2, width = 6.73, height = 9.17, units = "in", device = "jpg")
```
